...
# lastPhotoReadTextFile = thisScript + "currentData/lastPhotoRead.txt" # Not here anymore
...
def copySD(src, srcHD, sdFolderFile, srcSDfolder, lastPhotoFilename, lastPhotoReadTextFile, thisScript) 
  ...
  begin
      fileNow = File.open(lastPhotoReadTextFile, "w") { |file| file.write(fileSDbasename) } # must use explicit path, otherwise will use wherever we are are on the SD card
      # fileNow.puts fileSDbasename
      fileNow.close
      puts "\n#{lineNum}. The last file processed. fileSDbasename, #{fileSDbasename}, written to  #{fileSDbasename}. Changed this 2019.05.08 so may be messed up."
  rescue IOError => e
...
def copyAndMove(srcHD,destPhoto,destOrig)
  ...
  Dir.entries(srcHD).sort.each do |item| # This construct goes through each in order. Sometimes the files are not in order with Dir.foreach or Dir.entries without sort
    ...
    # next if ignoreNonFiles(item) == true # using above instead. Why. Mayb wasn't working.
    
    # I think the following ~20 lines is about not bringing jpg's into whatever program I'm using which is now Mylio. Now being used at the moment, so all commented out
...
def userCamCode(fn)
  ...
  case fileEXIF.model
    ...
    when "DMC-GX7" # no longer using, but may reprocess some photos
      userCamCode = ".gs.P" # gs for photographer. P for *P*anasonic Lumix
    when "DMC-TS5"
      userCamCode = ".gs.W" # gs for photographer. W for *w*aterproof Panasonic Lumix DMC-TS5. Camera is toast.
    else
      userCamCode = ".xx"
  end # case
...
def rename(src, timeZonesFile, timeNowWas)
  # src is destPhoto folder, i.e., where the photos were put by copy and move
  ...
  Dir.foreach(src) do |item| # for each photo file
    ...
    # puts "#{lineNum}. fn: #{src}/#{item}. will be renamed. " # #{timeNowWas = timeStamp(timeNowWas)}
    fn = src + item # long file name
    # puts "#{lineNum}. fn: #{fn} another way" # error checking.
    ...
    if File.file?(fn)
      ...
      # fileSubSecDateTimeOriginal = fileEXIF.SubSecDateTimeOriginal # This doesn't show the subsec, same as dateTimeOriginal. Shows correctly in iTerm. Bottom line, can't use in this script
 ...
 #    puts "#{lineNum}. fileDateTimeOriginal: #{fileDateTimeOriginal}. SubSecTimeOriginal: #{fileEXIF.SubSecTimeOriginal} and exists #{subSecExist}"
...
#       puts "#{lineNum}: camModel: #{camModel}. tzoLoc: #{tzoLoc}. timeChange.class: #{timeChange.class} timeChange: #{timeChange.to_i}"
      ...
      oneBack = fileDate == fileDatePrev # && fileExt == fileExtPrev # true if previous file at the same time calculated in local time. Ig
      puts "#{lineNum}. #{oneBack}. fn-filePrev:  #{item} - #{filePrev}. fileDate$fileDatePrev: #{fileDate} $ #{fileDatePrev}. fileExt-fileExtPrev: #{fileExt}.- #{fileExtPrev}. #{fileExt == fileExtPrev} < fileExt == fileExtPrev "
  puts "#{lineNum}.. oneBack: #{oneBack}. item: #{item}. filePrev: #{filePrev}"
      if oneBack
        if subSecExists # mainly GX8
          fileBaseName = fileDate.strftime("%Y.%m.%d-%H.%M.%S") + "." + fileSubSecTimeOriginal.to_s + userCamCode(fn)
          if fileExt == fileExtPrev && dupCount == 0
            # Go back one and NO that isn't how this flow is going and the info fn and other file specific info has changed. Unless have a prev for all that info: fn, fileEXIF, fileDateTimeOrignalstr, tzoLoc, fileDate, fileSubSecTimeOriginal. All the stuff that feeds into 474 to 478. This shouldn't be too hard, but seems heavy handed. 
          end
        else # Most files 
          dupCount =+ 1
...
puts "#{lineNum}. destPhoto: #{destPhoto}. Does it have a slash?"
destOrig  = prefsPhoto["destOrig"] # + "/"
# Above are true whether SD or from another folder. Had two, so took out added one
...
# whichOne =="SD" ? unmountCard(sdCard) : "" # not working so commented out TODO
